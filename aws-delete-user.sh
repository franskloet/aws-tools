...
if ! aws iam get-user --user-name "$USERNAME" &>/dev/null; then
    echo "WARNING: User '$USERNAME' does not exist in AWS IAM"
    echo "Will only clean up local AWS configuration files"
    USER_EXISTS=false
else
...
if [ "$USER_EXISTS" = true ]; then
    ...
    echo ""
    echo "Skipping IAM operations (user does not exist in AWS)"
fi
...
if [ "$USER_EXISTS" = true ]; then
    ...
    echo "Step 1: Removing profile from AWS configuration..."
fi
